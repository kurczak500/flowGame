...
func release_rocket():
	if mass > mass_rocket:
		...
		new_rocket.velocity = Vector2(0.0, 0.0)
		...
		#1st
		#new_rocket.velocity = velocity#((( mass + mass_rocket ) * velocity) - (mass * velocity)) / mass_rocket
		#2nd
		new_rocket.velocity = -velocity
		velocity = ((mass + mass_rocket) * velocity - mass_rocket * velocity)/mass  
		#3rd
